...
exports.encodeSignedDeviceIdentity = exports.configureSuccessfulPairing = exports.generateMobileLoginNode = exports.generateMobileNode = exports.generateRegistrationNode = exports.generateLoginNode = void 0;
...
const getMobileUserAgent = (config) => {
    const device = DeviceConfig_1.getDeviceConfig();
    const platform = device.business 
        ? Defaults_1.PLATFORM.ANDROID_BUSINESS 
        : (device.platform !== undefined ? device.platform : Defaults_1.PLATFORM.ANDROID);
    ...
    return {
        ...
        localeLanguageIso6391: 'en',
        localeCountryIso31661Alpha2: config.countryCode || 'US',
        deviceType: WAProto_1.proto.ClientPayload.UserAgent.DeviceType.PHONE,
        manufacturer: device.manufacturer,
        deviceModelType: device.deviceModelType || 2,
    };
};
...
const getMobileClientPayload = (config) => {
    const payload = {
        connectType: WAProto_1.proto.ClientPayload.ConnectType.WIFI_UNKNOWN,
        connectReason: WAProto_1.proto.ClientPayload.ConnectReason.USER_ACTIVATED,
        userAgent: getMobileUserAgent(config),
    };
    return payload;
};
...
const generateMobileNode = (config) => {
    ...
    const appVersionBuf = (0, crypto_1.createHash)('md5')
        .update(config.version.join('.'))
        .digest();
    const identPub = creds.signedIdentityKey.public;
    // Strip 0x05 prefix for mobile - server expects raw 32-byte key
    const eIdent = Buffer.isBuffer(identPub) && identPub.length === 33 && identPub[0] === 0x05
        ? identPub.slice(1)
        : identPub;
    const skeyPub = creds.signedPreKey.keyPair.public;
    const eSkeyVal = Buffer.isBuffer(skeyPub) && skeyPub.length === 33 && skeyPub[0] === 0x05
        ? skeyPub.slice(1)
        : skeyPub;
    const registerPayload = {
        ...getMobileClientPayload(config),
        passive: false,
        pull: false,
        devicePairingData: {
            buildHash: appVersionBuf,
            ...
            eKeytype: Defaults_1.KEY_BUNDLE_TYPE,
            eIdent: eIdent,
            eSkeyId: (0, generics_1.encodeBigEndian)(creds.signedPreKey.keyId, 3),
            eSkeyVal: eSkeyVal,
...
const generateMobileLoginNode = (userJid, config) => {
    const { user, device } = (0, WABinary_1.jidDecode)(userJid);
    const { creds } = config.auth;
    const pushName = (creds.me && creds.me.name && creds.me.name !== 'User') ? creds.me.name : undefined;
    const payload = {
        ...getMobileClientPayload(config),
        passive: false,
        pull: false,
...
exports.generateMobileLoginNode = generateMobileLoginNode;
...
const encodeSignedDeviceIdentity = (account, includeSignatureKey) => {
    if (!account) return Buffer.alloc(0);
    account = { ...account };
