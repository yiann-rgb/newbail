...
const makeNoiseHandler = ({ keyPair: { private: privateKey, public: publicKey }, NOISE_HEADER, mobile, logger, routingInfo }) => {
    ...
    return {
        ...
        encodeFrame: (data) => {
            ...
            // Mobile mode: NOISE_HEADER goes in the hash only, not on the wire.
            // Web mode: routingInfo may prefix the frame.
            let header;
            if (!mobile && routingInfo) {
            ...
            const introSize = (sentIntro || mobile) ? 0 : (header ? header.length : 0);
            const frame = Buffer.alloc(introSize + 3 + data.byteLength);
            if (!sentIntro && !mobile && header) {
                frame.set(header);
                sentIntro = true;
            }
