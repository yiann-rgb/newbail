...
const makeSocket = (config) => {
    ...
    if (isMobile) {
        // Mobile mode: connect to g.whatsapp.net:443 via raw TCP
        const mobileUrl = new url_1.URL('tcp://' + Defaults_1.MOBILE_ENDPOINT + ':' + Defaults_1.MOBILE_PORT);
        Object.assign(url, mobileUrl);
    } else if (url.protocol === 'tcp:') {
        throw new boom_1.Boom('Mobile API not supported without config.mobile=true', { 
            statusCode: Types_1.DisconnectReason.loggedOut 
        });
    }
    if (!isMobile && url.protocol === 'wss' && ((_a = authState === null || authState === void 0 ? void 0 : authState.creds) === null || _a === void 0 ? void 0 : _a.routingInfo)) {
        url.searchParams.append('ED', authState.creds.routingInfo.toString('base64url'));
    }
    const ws = isMobile ? new Client_1.MobileSocketClient(url, config) : new Client_1.WebSocketClient(url, config);
    ws.connect();
    ...
    const noise = (0, Utils_1.makeNoiseHandler)({
        ...
        NOISE_HEADER: isMobile ? Defaults_1.MOBILE_NOISE_HEADER : Defaults_1.NOISE_WA_HEADER,
        mobile: isMobile,
        logger,
        routingInfo: isMobile ? undefined : ((_b = authState === null || authState === void 0 ? void 0 : authState.creds) === null || _b === void 0 ? void 0 : _b.routingInfo)
    });
    ...
    const validateConnection = async () => {
        ...
        let node;
        if (!creds.me) {
            node = isMobile ? (0, Utils_1.generateMobileNode)(config) : (0, Utils_1.generateRegistrationNode)(creds, config);
            logger.info({ node }, 'not logged in, attempting registration...');
        }
        else {
            node = isMobile ? (0, Utils_1.generateMobileLoginNode)(creds.me.id, config) : (0, Utils_1.generateLoginNode)(creds.me.id, config);
    ...
    const requestPairingCode = async (phoneNumber, pairKey) => {
        ...
        // Ensure we have a pairing ephemeral key pair
        ...
        // For mobile, use proper device platform display
        const device = DeviceConfig_1.getDeviceConfig();
        const platformDisplay = isMobile 
            ? `${device.manufacturer} ${device.model}`
            : `${browser[1]} (${browser[0]})`;
        const platformId = isMobile
            ? (DeviceConfig_1.PLATFORM.ANDROID === device.platform ? '49' : '49') // Android phone
            : (0, Utils_1.getPlatformId)(browser[1]);
    ...
    ws.on('CB:ib,,downgrade_webclient', () => {
        if (!isMobile) {
            ...
            logger.info('ignoring downgrade_webclient in mobile mode');
        }
    });
